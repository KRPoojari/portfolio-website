...
// Mobile Menu Toggle with Modal Overlay
document.addEventListener('DOMContentLoaded', function () {
  ...
  let modalOverlay = document.getElementById('nav-modal-overlay');
  if (!modalOverlay) {
    modalOverlay = document.createElement('div');
    modalOverlay.id = 'nav-modal-overlay';
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const svgElements = document.querySelectorAll(
    '.gallery-item img, .project-image, .timeline-icon img'
  );
...
function initParallaxEffect() {
  ...
  if (heroImage) {
    document.addEventListener('mousemove', function (e) {
      ...
      heroImage.style.transform = `translate(${mouseX * 20}px, ${
        mouseY * 20
      }px)`;
    });
  }
